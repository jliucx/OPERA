...
covariate_full <- readRDS(inputs[["covariate_full"]])
...
covariate_full <- covariate_full[-cell_to_filter, ]
grna_assignment_matrix <- grna_assignment_matrix[, -cell_to_filter, drop = FALSE]
}
response_matrix <- t(t(response_matrix_raw) / covariate_full[, "response_n_umis"])
